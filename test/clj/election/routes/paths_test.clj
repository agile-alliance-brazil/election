...
(deftest place-vote-path-test
  ...
  (testing (str "place-vote-path with election-id and token is " paths/place-vote-matcher)
    (is (= "/1/vote/token" (paths/place-vote-path 1 "token")))
    (is (= "/1/vote/token" (paths/place-vote-path "1" "token")))
    (is (= "/alpha/vote/token" (paths/place-vote-path "alpha" "token")))
    (is (= "/1/vote/abc-def-ghi" (paths/place-vote-path 1 "abc-def-ghi")))
  )
)
